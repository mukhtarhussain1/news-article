import image4 from "../assets/images/image4.png";
import image5 from "../assets/images/image5.png";
import image6 from "../assets/images/image6.png";
import image7 from "../assets/images/image7.png";
import image8 from "../assets/images/image8.png";
import image9 from "../assets/images/image9.png";
import image10 from "../assets/images/image10.png";
import image11 from "../assets/images/image11.png";
import image12 from "../assets/images/image12.png";
import image13 from "../assets/images/image13.png";
import image14 from "../assets/images/image14.png";
import image15 from "../assets/images/image15.png";
import image16 from "../assets/images/image16.png";
import image18 from "../assets/images/image18.png";
import image19 from "../assets/images/image19.png";
import image20 from "../assets/images/image20.png";
import image22 from "../assets/images/image22.png";
import image23 from "../assets/images/image23.png";
import image24 from "../assets/images/image24.png";
import image25 from "../assets/images/image25.png";
import image26 from "../assets/images/image26.png";
import image27 from "../assets/images/image27.png";
import image28 from "../assets/images/image28.png";
import image29 from "../assets/images/image29.png";
import image30 from "../assets/images/image30.png";
import image31 from "../assets/images/image31.png";
import image32 from "../assets/images/image32.png";
import image33 from "../assets/images/image33.png";
import image34 from "../assets/images/image34.png";

export const sectionOneData = [
  {
    mainHeading:
      "Louvre Abu Dhabi marks 4th anniversary with world class exhibitions, programming for all",
    loram:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit.Phasellus in odio vel risus dignissim interdum.",
    auther: "U. R. Oliver",
    meals: "Meals on wheels best food trucks in the UAE",
    travel: "Travel",
    dine: "Dine",
    openHouse: "OPEN HOUSE",
    phasellus:
      "Phasellus in odio vel risus dignissim interdum. Sed vulputate pretium lobortis.",
    uae: "UAE: CBSE Term 1 exams begin tomorrow; important guidelines announced",
    worldExpo:
      "World Expo in Dubai: MP delegation holds discussion over film city and institute",
  },
];

export const sectionTwoData = [
  {
    image: image4,
    title: "Vladimir Putin",
    text: "Meals on wheels best in the UAE",
  },
  {
    image: image5,
    title: "Vladimir Putin",
    text: "Meals on wheels best in the UAE",
  },
  {
    image: image6,
    title: "Vladimir Putin",
    text: "Meals on wheels best in the UAE",
  },
  {
    image: image7,
    title: "Vladimir Putin",
    text: "Meals on wheels best in the UAE",
  },
  {
    image: image8,
    title: "Vladimir Putin",
    text: "Meals on wheels best in the UAE",
  },
];

export const sectionThreeData1 = [
  {
    image: image9,
    title: "Travel",
    text: "Louvre Abu Dhabi marks 4th anniversary with world class exhibitions,programming for all",
    text2:
      "Exhin imlave asfear asfasas afsfeaarsa asfasfasf asfasfa ten cultable doice inverum",
    span: "Cvita Doleschall",
  },
];

export const sectionThreeData2 = [
  {
    image: image10,
    title: "Culture",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
  {
    image: image11,
    title: "Culture",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
  {
    image: image12,
    title: "Culture",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
];

export const sectionFourData = [
  {
    image: image13,
    title: "Omar Mukhtar",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    auther: "U. R. Oliver",
  },
  {
    image: image14,
    title: "Lil Larry",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    auther: "U. R. Oliver",
  },
  {
    image: image15,
    title: "Lubna Hamdan",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    auther: "U. R. Oliver",
  },
  {
    image: image16,
    title: "Shawn Jhon",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    auther: "U. R. Oliver",
  },
];

export const sectionSixData = [
  {
    image: image18,
    title: "OPEN HOUSE",
    text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus in odio vel.",
    auther: "U. R. Oliver",
  },
  {
    image: image19,
    title: "OPEN HOUSE",
    text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus in odio vel.",
    auther: "U. R. Oliver",
  },
  {
    image: image20,
    title: "OPEN HOUSE",
    text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus in odio vel.",
    auther: "U. R. Oliver",
  },
];

export const sectionEightData = [
  {
    image: image31,
    ratings: "4.4",
    title: "Omar Mukhtar",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    span: "U. R. Oliver",
  },
  {
    image: image32,
    ratings: "4.4",
    title: "Lil Larry",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    span: "U. R. Oliver",
  },
  {
    image: image33,
    ratings: "4.4",
    title: "Lubna Hamdan",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    span: "U. R. Oliver",
  },
  {
    image: image34,
    ratings: "4.4",
    title: "Shawn Jhon",
    text: "Meals on wheels: Meals on wheels: best in the UAE best in Meals on wheels: best in",
    span: "U. R. Oliver",
  },
];

export const sectionNineData1 = [
  {
    image: image22,
    title: "keto kebabs",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
  {
    image: image23,
    title: "italian deserts",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
  {
    image: image24,
    title: "japanese flavors",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
];
export const sectionNineData2 = [
  {
    image: image27,
    title: "vegan meals",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
  {
    image: image26,
    title: "french pasta",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
  {
    image: image25,
    title: "korean bbq",
    text: "Meals on wheels best food trucks",
    text2: "in the UAE",
    span: "U. R. Oliver",
  },
];

export const sectionTenData = [
  {
    image: image28,
    title: "Eat",
    text: "Meals on wheels: Meals on wheels: best in the UAE best",
    text2: "in the UAE",
    auther: "U. R. Oliver",
  },
  {
    image: image29,
    title: "Eat",
    text: "Meals on wheels: Meals on wheels: best in the UAE best",
    text2: "in the UAE",
    auther: "U. R. Oliver",
  },
  {
    image: image30,
    title: "Eat",
    text: "Meals on wheels: Meals on wheels: best in the UAE best",
    text2: "in the UAE",
    auther: "U. R. Oliver",
  },
];
